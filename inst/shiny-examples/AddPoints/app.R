...
ui <- dashboardPage(
    ...
    # Header ---------
    dashboardHeader(title = "Design Builder"
    ...
    dashboardSidebar(
        sidebarMenu(id="sidebar",
            ...
            menuItem("Set up & Optimal design", tabName = "optdes", icon = icon("calculator")),
            menuItem("Custom design", tabName = "custom", icon = icon("clipboard"),
                     menuSubItem("Design Proposal", tabName = "givedes"),
                     menuSubItem("Restricted design", tabName = "restricteddes")),
            menuItem("Your design", tabName = "your", icon = icon("clipboard"),
                     menuSubItem("Build your design", tabName = "builddes"),
                     menuSubItem("Enhance your design", tabName = "enhacedes")
            )
        ...
        conditionalPanel(condition="input.sidebar == 'optdes'",
            selectizeInput(
            'model',  HTML(paste("Inputs:",'<br/>',"Choose a model")), choices = c("Antoine Equation", "Quadratic", "Quadratic Heteroscedastic", "Michaelis-Menten"#, "A-Optimality", "Ds-Optimality", "I-Optimality"
                                                           ),
            options = list(
            ...
            conditionalPanel("input.model == 'Antoine Equation' || input.model == 'Quadratic'",
                sliderInput("design_sp", "Design Space",
                        min = -100, max = 374, value = c(1, 100)
                )
            ),
            conditionalPanel("input.model == 'Michaelis-Menten'",
                             sliderInput("design_sp_MM", "Design Space: [0, value]",
                                         min = 0.01, max = 100, value = 10
                             )
            ),
            conditionalPanel("input.model == 'Antoine Equation'",
            ...
            conditionalPanel("input.model == 'Michaelis-Menten'",
                             numericInput('k_par', 'Initial value for parameter K', value = 227.27, step = 0.01
                             ),
                             numericInput('v_par', 'Initial value for parameter V', value = 43.73, step = 0.01
                             )
            ),
            conditionalPanel("input.model == 'Quadratic Heteroscedastic'",
                             sliderInput('order', 'Order of the polynomial', min = -0.99, max = 1, value = 0.5
                             ),
                             sliderInput("design_sp_CH", "Design Space: [0, value]",
        ...
        #                  )
        # ),
        conditionalPanel(condition="input.sidebar == 'givedes' || input.sidebar =='restricteddes'",
            sliderInput("delta", HTML(paste("Inputs:",'<br/>',withMathJax("Weight of the new point(s), alpha"))),
                        min = 0, max = 0.99, value = 0
            ),
            uiOutput("eff"),
            actionButton(inputId = "give_points", "Region of Candidate Points"
            )
        ),
        conditionalPanel("input.sidebar == 'builddes'",
            numericInput('points_add',  HTML(paste("Inputs:",'<br/>',"1. Point")), value = -101, min = -100, max = 374, step = 1
            ),
            conditionalPanel('input.points_add != -101',
                             numericInput('weights_add', '2. Weight', value = 0, min = 0, max = 1, step = 0.01)
            ),
            actionButton("add", "Add point"
            ),
            actionButton("remove", "Remove last point"
            ),
            actionButton("remove_all", "Remove all points"
            ),
            actionButton("both_designs", "Calculate efficiency"
            )
            ),
        conditionalPanel("input.sidebar == 'restricteddes'",
            conditionalPanel("output.show_rest == 1",
                 numericInput('points_add_opt', '1. Point', value = -101, min = -100, max = 374, step = 1
                 ),
                 conditionalPanel('input.points_add_opt != -101',
                                  numericInput('weights_add_opt', '2. Weight', value = 0, min = 0, max = 1, step = 0.01)
                 ),
                 actionButton("add_opt", "Add point"
                 ),
                 actionButton("remove_opt", "Remove last point"
                 ),
                 actionButton("remove_opt_all", "Remove all points"
                 ),
                 actionButton("with_opt", "Calculate efficiency"
                 )
             )
        ),
        conditionalPanel(condition="input.sidebar == 'enhacedes'",
                 sliderInput("deltaenh",  HTML(paste("Inputs:",'<br/>',"Weight of the new point(s)")),
                             min = 0, max = 0.99, value = 0
                 ),
                 uiOutput("effenh"),
                 actionButton("enhace_points", "Region of Candidate Points"
                 ),
                 conditionalPanel("output.show_enh == 1",
                     numericInput('points_add_enh', '1. Point', value = -101, min = -100, max = 374, step = 1
                     ),
                     conditionalPanel('input.points_add_enh != -101',
                                      numericInput('weights_add_enh', '2. Weight', value = 0, min = 0, max = 1, step = 0.01)
                     ),
                     actionButton("add_enh", "Add point"
                     ),
                     actionButton("remove_enh", "Remove last point"
                     ),
                     actionButton("remove_enh_all", "Remove all points"
                     ),
                     actionButton("enhace_design", "Calculate efficiency"
                     )
                 )
        ),
        conditionalPanel("input.sidebar == 'givedes'",
            #span("Test", style="color:white"),
            actionButton("stand_design", "Calculate design"
        ))),
    ...
    dashboardBody(
        ...
        tabItems(
            ...
            tabItem(tabName = "presentation", 
                    fluidPage(withMathJax(includeMarkdown("presentacion.Rmd")))),
            # Optimal Design
